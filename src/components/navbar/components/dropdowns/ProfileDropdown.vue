...
const resolveLinkAttribute = (item: ProfileListItem) => {
  // Prevent navigation attribute for logout so click handler runs exclusively
...
const handleLogout = async () => {
  try {
    await logout()
    notify({ message: t('user.logout'), color: 'success' })
    router.push({ name: 'login' })
  } catch (e) {
    notify({ message: t('user.logoutError') || 'Logout failed', color: 'danger' })
  }
}
</script>
